  ...
  navigator.geolocation.getCurrentPosition(async position => {
    ...
    lon = position.coords.longitude;
     // lat = fixLat.toFixed(2);
      ...
      const city_url = `weather/${lat},${lon}`;
      console.log(city_url);
      const city_res = await fetch(city_url);
      console.log(city_res);
      const city_json = await city_res.json();

      console.log(city_json.city_loc);
      

      document.getElementById('city').textContent = city_json.city_loc.address.city;
      document.getElementById('city1').textContent = city_json.city_loc.address.city;
      document.getElementById('city2').textContent = city_json.city_loc.address.city;
      document.getElementById('city3').textContent = city_json.city_loc.address.city;
      document.getElementById('city4').textContent = city_json.city_loc.address.city;
      document.getElementById('city5').textContent = city_json.city_loc.address.city;
